...
const Form = forwardRef((props, ref) => {
  ...
  return (
    <div
      className={errors[name] ? `${className} ${styles.formError}` : className}
    >
      ...
      {errors[name] && errors[name].type === 'required' && (
        <span className={styles.labelError}>
          {intl.messages[`common.${name}Required`]}
        </span>
      )}
      {errors[name] && errors[name].type === 'minLength' && (
        <span className={styles.labelError}>
          {intl.messages[`common.${name}Invalid`]}
        </span>
      )}
